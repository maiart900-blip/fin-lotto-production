...
export async function GET(request: NextRequest) {
  try {
    ...
    const topAgentIds = [...agentSales.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
      .map(([id]) => id);
    ...
    // Get risk numbers (high payout potential)
    const riskNumbers = [...numberCounts.entries()]
      .map(([number, data]) => {
        const payout = data.amount * 80; // Assume 80x payout rate
        ...
        return {
          ...
          payout,
          loss,
          risk: loss > totalSales * 0.5 ? 'high' : loss > totalSales * 0.2 ? 'medium' : 'low',
        };
      })
      .filter(n => n.risk !== 'low')
