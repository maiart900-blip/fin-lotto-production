...
export async function GET(request: NextRequest) {
  try {
    ...
    // Map commissions with agent info
    ...
    const stats = {
      ...
      avgCommission: mappedCommissions.length > 0 
        ? Math.round(mappedCommissions.reduce((sum, c) => sum + (c.commission_amount || 0), 0) / mappedCommissions.length)
        : 0,
    };
  ...
  try {
    ...
    if (action === 'approve') {
      ...
      await supabase.from('audit_logs').insert({
        ...
        entity_type: 'commission_transactions',
        entity_id: id,
      });
    ...
    if (action === 'pay') {
      ...
      // Get commission details to update agent balance
      ...
      if (commission) {
        ...
        await supabase
          .from('users')
          .update({ credit_balance: (agent?.credit_balance || 0) + commission.commission_amount })
          .eq('id', commission.agent_id);
      }
