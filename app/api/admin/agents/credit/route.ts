...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Update balance using wallet service pattern with optimistic locking
    ...
    try {
      await supabase.from('credit_transactions').insert({
        ...
        description: note || `${action === 'add' ? 'เติม' : 'ตัด'}เครดิตโดยผู้ดูแลระบบ`,
        status: 'completed',
    ...
    // Audit log
    await auditLogger.logFinancial('system', {
      action: action === 'add' ? 'agent_credit_add' : 'agent_credit_deduct',
      description: `${action === 'add' ? 'เติม' : 'ตัด'}เครดิตเอเย่นต์ ${agent.name || agent.username} จำนวน ฿${numAmount.toLocaleString()}`,
      details: {
        agent_id,
