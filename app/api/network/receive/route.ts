...
interface SyncPayload {
  type: 'entries_sync' | 'lottery_status' | 'payout_rates' | 'blocked_numbers' | 'market_close' | 'full_sync';
...
function verifyApiKey(request: NextRequest): boolean {
  ...
  const expectedKey = process.env.NETWORK_API_KEY || process.env.PARENT_SITE_API_KEY;
    ...
    switch (type) {
      case 'entries_sync': {
        ...
        return NextResponse.json({
          ...
          total_amount,
          message: `Received ${entries.length} entries from ${source_site_id || childSiteId}`,
        });
      }
      ...
      case 'lottery_status': {
        ...
        return NextResponse.json({ success: true, message: 'Lottery status updated' });
      }
      ...
      case 'payout_rates': {
        ...
        return NextResponse.json({ success: true, updated: rates.length });
      }
      ...
      case 'blocked_numbers': {
        ...
        if (!blocked_numbers || !Array.isArray(blocked_numbers)) {
          return NextResponse.json({ error: 'Invalid blocked_numbers data' }, { status: 400 });
        }
        ...
        return NextResponse.json({ success: true, updated: blocked_numbers.length });
      }
      ...
      case 'full_sync': {
        ...
        const { lotteries, payout_rates, blocked_numbers } = data;
        
        let synced = { lotteries: 0, rates: 0, blocked: 0 };
        
        if (lotteries && Array.isArray(lotteries)) {
          for (const lottery of lotteries) {
            await supabase.from('lotteries').upsert(lottery, { onConflict: 'id' });
            synced.lotteries++;
          }
        }
