...
export async function POST(request: NextRequest) {
  try {
    ...
    entriesData.forEach((entry) => {
      const lottery = entry.lottery as { name: string; draw_date: string } | null;
      ...
      aggregations[key].payout_liability += Number(entry.potential_payout) || 0;
      aggregations[key].bet_count++;
...
export async function GET() {
  return NextResponse.json({
    ...
    method: 'POST',
    description: 'Aggregates key-in betting data into risk_aggregations table',
