...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[CRON] Daily Risk Reset starting at ${now.toISOString()}`);
    console.log(`[CRON] Archiving data for: ${yesterdayBusinessDay}, New day: ${businessDay}`);
    
    // ========================================================
    // STEP 1: Archive yesterday's risk data to history table
    // ========================================================
    
    // Get all risk aggregations for yesterday grouped by lottery_type
    ...
    if (fetchError) {
      console.error('[CRON] Error fetching yesterday data:', fetchError);
    }
    
    let archivedCount = 0;
    
    if (yesterdayData && yesterdayData.length > 0) {
      // Group by lottery_type for summary
      const byLotteryType: Record<string, {
        ...
        high_count: number;
        medium_count: number;
        top_exposures: Array<{ number: string; bet_type: string; liability: number; sources: string[] }>;
        source_breakdown: Record<string, number>;
      }> = {};
      
      yesterdayData.forEach((row) => {
        const lotteryType = row.lottery_type;
        ...
        group.total_bet_amount += Number(row.total_bet_amount) || 0;
        group.total_payout_liability += Number(row.payout_liability) || 0;
        group.total_bet_count += row.bet_count || 0;
        group.unique_numbers.add(row.lottery_number);
        
        // Count risk levels
        if (row.risk_level === 'critical') group.critical_count++;
        else if (row.risk_level === 'high') group.high_count++;
        else if (row.risk_level === 'medium') group.medium_count++;
        
        // Track source breakdown
        const source = row.source_type;
        group.source_breakdown[source] = (group.source_breakdown[source] || 0) + Number(row.total_bet_amount);
        
        // Add to top exposures (sorted later)
        group.top_exposures.push({
          number: row.lottery_number,
          bet_type: row.bet_type,
          liability: Number(row.payout_liability) || 0,
          sources: [row.source_site_name || row.source_type],
        });
      });
      
      // Insert archive records
      const archiveRecords = Object.entries(byLotteryType).map(([lotteryType, data]) => ({
      ...
      const { error: archiveError } = await supabase
        ...
        .upsert(archiveRecords, {
          onConflict: 'snapshot_date,lottery_type,draw_date',
          ignoreDuplicates: false,
        });
      
      if (archiveError) {
        console.error('[CRON] Error archiving data:', archiveError);
      } else {
        archivedCount = archiveRecords.length;
        console.log(`[CRON] Archived ${archivedCount} lottery type summaries`);
      }
    }
    ...
    // Delete yesterday's detailed aggregations (now archived)
    // Keep only last 7 days of detailed data for real-time queries
    ...
    if (deleteError) {
      console.error('[CRON] Error cleaning old aggregations:', deleteError);
    } else {
      console.log(`[CRON] Deleted ${deletedCount || 0} old aggregation records (before ${cutoffDate})`);
    }
    ...
    console.error('[CRON] Daily risk reset error:', error);
    return NextResponse.json({ 
...
export async function GET() {
  return NextResponse.json({
    ...
    method: 'POST',
    description: 'Resets daily risk counters at 01:00 AM Thailand time and archives historical data',
    schedule: '0 18 * * * (18:00 UTC = 01:00 Thailand)',
