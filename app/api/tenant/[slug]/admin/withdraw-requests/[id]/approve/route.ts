...
export async function POST(
  ...
  if (withdrawRequest.status !== 'pending' && withdrawRequest.status !== 'pending_second_approval') {
  ...
  const requiresDualApproval = withdrawRequest.amount >= DUAL_APPROVAL_THRESHOLD;
  
  if (requiresDualApproval && !withdrawRequest.approved_by) {
    ...
    await supabase.from('audit_logs').insert({
      user_id: approverId,
      action: 'withdraw_first_approval',
      entity_type: 'tenant_withdraw_requests',
      entity_id: id,
      details: {
  ...
  if (withdrawRequest.requires_dual_approval && withdrawRequest.approved_by) {
    // Prevent same person from second approval
    if (withdrawRequest.approved_by === approverId) {
      return NextResponse.json({ 
  ...
  // Update withdraw request status - final approval
  const { error: updateError } = await supabase
    .from('tenant_withdraw_requests')
    .update({ 
      status: 'approved',
      approved_at: requiresDualApproval ? withdrawRequest.approved_at : new Date().toISOString(),
      approved_by: requiresDualApproval ? withdrawRequest.approved_by : approverId,
      approved_ip: requiresDualApproval ? withdrawRequest.approved_ip : clientIP,
  ...
  await supabase.from('transactions').insert({
    ...
    status: 'completed',
    description: requiresDualApproval ? 'ถอนเงิน (อนุมัติ 2 ขั้น)' : 'ถอนเงิน (อนุมัติ)',
    reference_id: id,
  });
  ...
  await supabase.from('audit_logs').insert({
    user_id: approverId,
    action: requiresDualApproval ? 'withdraw_second_approval' : 'withdraw_approved',
    entity_type: 'tenant_withdraw_requests',
    entity_id: id,
    details: {
      amount: withdrawRequest.amount,
      customer_id: withdrawRequest.customer_id,
      ip: clientIP,
      first_approver: withdrawRequest.approved_by,
    },
    created_at: new Date().toISOString(),
  }).catch(() => {});
