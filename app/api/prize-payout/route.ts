  ...
  try {
    ...
    let query = supabase
      ...
      .select(`
        ...
        customer_id,
        customer:customers(id, name, phone, bank_code, bank_account_number, bank_account_name)
      `)
      .eq('status', 'won')
    ...
    payoutRates?.forEach(r => {
      rateMap.set(`${r.lottery_id}-${r.bet_type}`, parseFloat(r.rate));
    });
    
    // เพิ่มข้อมูล payout_amount ที่คำนวณจาก rate (ถ้ายังไม่มี)
    const enrichedEntries = (entries || []).map(entry => {
      let calculatedPayout = entry.payout_amount || 0;
      
      // ถ้า payout_amount เป็น 0 ให้คำนวณจาก rate
      if (calculatedPayout === 0 && entry.lottery_id && entry.bet_type) {
        const rate = rateMap.get(`${entry.lottery_id}-${entry.bet_type}`) || 0;
        calculatedPayout = (entry.amount || 0) * rate;
      }
      
      return {
        ...entry,
        payout_amount: calculatedPayout,
        // เพิ่ม customer_name จาก customer object (ถ้ามี)
    ...
    const stats = {
      ...
      total_payout: enrichedEntries.reduce((sum, e) => sum + (e.payout_amount || 0), 0),
      pending_count: enrichedEntries.filter(e => !e.payout_status || e.payout_status === 'pending').length,
      paid_count: enrichedEntries.filter(e => e.payout_status === 'paid').length,
    };
    
    return NextResponse.json({ entries: enrichedEntries, stats });
  } catch (error) {
    console.error('Error in prize-payout GET:', error);
    return NextResponse.json({ entries: [], error: 'Internal server error' }, { status: 500 });
  }
}
