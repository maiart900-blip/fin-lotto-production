...
import { BarChart3, ArrowLeft, RefreshCw, Calendar, TrendingUp, TrendingDown, Users, DollarSign, Download } from 'lucide-react';
import Link from 'next/link';
import useSWR from 'swr';

const fetcher = (url: string) => fetch(url).then(r => r.json());

export default function ComparisonReportDailyPage() {
  const [date, setDate] = useState(new Date().toISOString().split('T')[0]);
  const { data, mutate, isLoading } = useSWR(`/api/reports/comparison/daily?date=${date}`, fetcher);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><BarChart3 className="size-6 text-[#D4AF37]" />รายงานเปรียบเทียบรายวัน</h1>
            <p className="text-slate-400 mt-1">เปรียบเทียบยอดระหว่างออโต้และคีย์หวย</p>
          </div>
        </div>
        <div className="flex gap-2">
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="px-3 py-2 rounded-md bg-slate-800 border border-slate-700 text-white" />
      ...
      <Card className="bg-black/40 border-[#D4AF37]/20">
        <CardHeader><CardTitle className="text-white flex items-center gap-2"><Calendar className="size-5 text-[#D4AF37]" />เปรียบเทียบ วันที่ {date}</CardTitle></CardHeader>
        <CardContent>
