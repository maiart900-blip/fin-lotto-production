...
export default function AutoMarketingCampaignsPage() {
  const [search, setSearch] = useState('');
  const [showDialog, setShowDialog] = useState(false);
  const [newCampaign, setNewCampaign] = useState({ name: '', description: '', budget: '' });
  const { data, mutate, isLoading } = useSWR('/api/marketing/campaigns?type=auto', fetcher);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Megaphone className="size-6 text-[#D4AF37]" />แคมเปญออโต้</h1>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="bg-slate-900 border-slate-700">
          ...
          <div className="space-y-4 py-4">
            <div><Label className="text-slate-300">ชื่อแคมเปญ</Label><Input value={newCampaign.name} onChange={(e) => setNewCampaign({...newCampaign, name: e.target.value})} placeholder="เช่น โปรโมชั่นสมัครใหม่" className="mt-2 bg-slate-800 border-slate-700" /></div>
