...
export default function AutoMarketingLinksPage() {
  const [search, setSearch] = useState('');
  const [showDialog, setShowDialog] = useState(false);
  const [newLink, setNewLink] = useState({ name: '', source: '' });
  const { data, mutate, isLoading } = useSWR('/api/marketing/links?type=auto', fetcher);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/auto-marketing"><Button variant="ghost" size="icon"><ArrowLeft className="size-5" /></Button></Link>
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Link2 className="size-6 text-[#D4AF37]" />ลิงก์สมัครออโต้</h1>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="bg-slate-900 border-slate-700">
          ...
          <div className="space-y-4 py-4">
            <div><Label className="text-slate-300">ชื่อลิงก์</Label><Input value={newLink.name} onChange={(e) => setNewLink({...newLink, name: e.target.value})} placeholder="เช่น Facebook Ads" className="mt-2 bg-slate-800 border-slate-700" /></div>
            <div><Label className="text-slate-300">Source Code</Label><Input value={newLink.source} onChange={(e) => setNewLink({...newLink, source: e.target.value})} placeholder="เช่น fb-ads-01" className="mt-2 bg-slate-800 border-slate-700" /></div>
          </div>
