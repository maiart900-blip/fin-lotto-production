...
export async function resolveAgentPermissions(
  ...
  if (agentPerms && agentPerms.length > 0) {
    ...
    try {
      visibleMenus =
        typeof agent.visible_menus === 'string'
  ...
  const canManualKey =
    agent.enable_manual_key !== false &&
    (
      tenantMode === 'manual_key_only' ||
      tenantMode === 'hybrid' ||
      tenantMode === 'both'
    );
  ...
  const canAutoSystem =
    agent.enable_auto === true &&
    (
      tenantMode === 'auto_only' ||
      tenantMode === 'hybrid' ||
      tenantMode === 'both'
    );
  ...
  effectiveMenus = effectiveMenus.filter(
    (menu) => !PLATFORM_ONLY_MENUS.includes(menu)
  );
  ...
  return {
    ...
    agent_tier: agent.agent_tier ?? agent.tier ?? null,
    agent_level: agent.agent_level ?? agent.level ?? null,
    can_manage_agents:
      agent.can_manage_agents ??
    ...
    disabled_features: tenantFeatures.filter(
    ...
    can_approve_transactions:
      permissionsMap['financial']?.can_approve ?? false,
    can_manage_members:
      permissionsMap['member-visibility']?.can_view ?? true,
    can_manage_finances:
      permissionsMap['financial']?.can_view ?? false,
  };
}